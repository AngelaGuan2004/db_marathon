<template>
  <div id="PackageManagement">
    <div class="management-container">
      <div style="margin-bottom: 50px;font-weight: bold;font-size: 26px;">{{ packageForm.eventName }}</div>
  ...
  methods: {
    ...
    handleSaveItems(selectedItems) {
      selectedItems.forEach(selectedItem => {
        const existingItem = this.items.find(item => item.id === selectedItem.id && item.kind === selectedItem.kind);
        if (existingItem) {
    ...
    async savePackageDetails() {
      const eventId = this.$route.params.event_id; // 固定的赛事ID
      // 过滤出新添加的物品
      const newItems = this.items.filter(item => item.source === '新添加');
