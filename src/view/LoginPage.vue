...
export default {
  ...
  data() {
    return {
      ...
      adminKey: '',
      role: '',
      showAthletePassword: false, // 控制选手密码显示/隐藏
      showPhotoPassword: false, // 控制摄影师密码显示/隐藏
      showAdminKey: false // 控制管理员密钥显示/隐藏
    }
  },
  computed: {
    isValidIdNumber() {
  ...
  methods: {
    register() {
      // 处理注册逻辑，例如 API 请求
      console.log('注册信息:', this.name, this.phone, this.idNumber, this.athletePassword, this.photoPassword, this.adminKey, this.role);
      // 根据身份进行跳转
      switch (this.role) {
        case 'admin':
