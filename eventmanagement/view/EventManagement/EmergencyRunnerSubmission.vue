...
export default {
  ...
  computed: {
    ...
    participants() {
    return this.$store.getters.getSelectedEmergencyRunners;
  }
  },
  methods: {
    checkClass(index) {
  ...
  removeFromSelectedEmergencyRunners(participant) {
    ...
    const index = this.$store.getters.getSelectedEmergencyRunners.findIndex(p => p.id === participant.id);
    if (index !== -1) {
      // 更新 Vuex store
      const updatedSelected = [...this.$store.getters.getSelectedEmergencyRunners];
      updatedSelected.splice(index, 1);
      this.$store.commit('setSelectedEmergencyRunners', updatedSelected);
    }
  }
  },
  created() {
    this.participants = this.$store.getters.getSelectedEmergencyRunners;
  }
};
</script>
<style>
