  ...
  <div id="ParticipantLottery">
    <el-main class="ParticipantLotteryContent">
      <div class="ParticipantLotteryInput">
        <el-input v-model="totalParticipants" placeholder="请输入总人数" class="InputTotalParticipants" :disabled="isDrawn === '是'"></el-input>
...
loadParticipants() {
            ...
            if (this.isDrawn === '是') {
              ...
              numberPromise = getNumberByEventIdAndPlayerId(player.player_Id, eventId)
                .then(res => {
                  ...
                  return res; // 假设返回的号码在 res.number_ 中
                })
                .catch(err => {
                  if (err.response) {
  ...
  const ballotData = [
    ...
    number_: p.number,
    role_: p.role,  // 直接使用 fetchPlayerDetails 得到的 role
    player_Id: p.id,
    event_Id: "10002"  // 固定的 event_Id
  }));

  // 调用 API 上传抽签结果
  uploadBallotResults(ballotData)
    .then(response => {
      this.$message.success('中签结果上传成功');
    })
    .catch(error => {
      console.error('Failed to upload ballot results:', error);
      this.$message.error('上传中签结果时出错，请稍后重试。');
    });
      // 抽签完成后更新 is_Drawn 字段
      updateEvent({
        id: "10002",
